...
sed -i '7s/.*/f = -x /' $MyFile
sed -i '4s/.*/ /' $MyPlotter
sed -i '6s/.*/set output \"plot2.eps\"  /' $MyPlotter
sed -i '14s/.*/plot \"data\" using 1:2  title \"Method\" /' $MyPlotter
...
sed -i '4s/.*/ /' $MyPlotter
sed -i '6s/.*/set output \"plot3.eps\" /' $MyPlotter
sed -i '14s/.*/plot \"data\" using 1:2  title \"Method\" /' $MyPlotter
...
sed -i '4s/.*/ /' $MyPlotter
sed -i '6s/.*/set output \"plot4.eps\" /' $MyPlotter
sed -i '14s/.*/plot \"data\" using 1:2  title \"Method\" /' $MyPlotter
make
(echo "100" | ./catenaire.out) > data
make plot
